...
export function generateLeadMagnetPDF(data: LeadMagnetPDFData): void {
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.text('Try FreshBooks - the #1 accounting software for freelancers and solopreneurs.', 20, y);
  
  doc.setFontSize(8);
  doc.setTextColor(148, 163, 184);
  doc.text('Generated by Safe Harbor 2025 Tax Shield | creatoraiplaybook.co', 105, 280, { align: 'center' });
  doc.text('This document is for informational purposes only and does not constitute tax advice.', 105, 285, { align: 'center' });
  doc.text(`Generated on ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`, 105, 290, { align: 'center' });
