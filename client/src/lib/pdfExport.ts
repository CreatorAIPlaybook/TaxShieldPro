...
export function generateLeadMagnetPDF(data: LeadMagnetPDFData): void {
  ...
  // Footer at absolute position at the bottom of the page (pageHeight - 20)
  const pageHeight = doc.internal.pageSize.getHeight();
  const footerY = pageHeight - 20;
  
  doc.setFontSize(8);
  doc.setTextColor(148, 163, 184);
  doc.text('Generated by Safe Harbor 2025 Tax Shield | creatoraiplaybook.co', 105, footerY, { align: 'center' });
  doc.text('This document is for informational purposes only and does not constitute tax advice.', 105, footerY + 5, { align: 'center' });
  doc.text(`Generated on ${new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}`, 105, footerY + 10, { align: 'center' });
